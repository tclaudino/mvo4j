...
public class StringUtils {
	...
	public static String toCammelCase(String txt) {
		return isNullOrEmpty(txt) ? "" : txt.substring(0, 1).toLowerCase()
				+ txt.substring(1);
	}

	public static String toBeanConvencionCase(String txt) {
		return isNullOrEmpty(txt) ? "" : txt.substring(0, 1).toUpperCase()
				+ txt.substring(1);
	}
	...
	public static String clearString(String body) {
		...
		body = body.replaceAll("[�?ÄÀÂÃ]", "a");
		body = body.replaceAll("[ÉËÈÊ]", "e");
		body = body.replaceAll("[�?�?ÌÎ]", "i");
		body = body.replaceAll("[ÓÖÒÔÕ]", "o");
		body = body.replaceAll("[ÚÜÙÛ]", "u");
		body = body.replaceAll("[Ç]", "c");
		body = body.replaceAll("[Ñ]", "n");
