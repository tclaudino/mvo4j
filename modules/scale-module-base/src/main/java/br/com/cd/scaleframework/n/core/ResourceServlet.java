...
public class ResourceServlet extends HttpServlet {
	...
	private OutputStream selectOutputStream(HttpServletRequest request,
		...
		if (gzipEnabled && StringUtils.hasText(acceptEncoding)
	...
	private void prepareResponse(HttpServletResponse response, URL[] resources,
		...
		for (int i = 0; i < resources.length; i++) {
			...
			if (currentMimeType == null) {
				String extension = resources[i].getPath().substring(
						resources[i].getPath().lastIndexOf('.'));
		...
		response.setHeader(HTTP_CONTENT_LENGTH_HEADER,
	...
	private URL[] getRequestResourceURLs(HttpServletRequest request)
		...
		for (int i = 0; i < localResourcePaths.length; i++) {
			...
			if (resource == null) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Searching classpath for resource: "
							+ jarResourcePath);
				}
				resource = ClassUtils.getDefaultClassLoader().getResource(
						jarResourcePath);
			}
			if (resource == null) {
	...
	private String rawResourcePath(HttpServletRequest request) {
		...
		if (rawResourcePath == null
				&& request instanceof ModuleHttpServletRequest) {
	 ...
	 * 
	 * @param seconds
	 *            number of seconds into the future that the response should be
	 *            cacheable for
	 */
	private void configureCaching(HttpServletResponse response, int seconds) {
		// HTTP 1.0 header
		response.setDateHeader(HTTP_EXPIRES_HEADER, System.currentTimeMillis()
				+ seconds * 1000L);
		// HTTP 1.1 header
	...
	/**
	 * Set whether to apply gzip compression to resources if the requesting
	 * client supports it.
	 */
	 ...
	 * "META-INF/** /*.js", The paths may be any Ant-style pattern parsable by
	 * AntPathMatcher.
	 * 
	 * @see AntPathMatcher
	 */
	 ...
	 * Typically, gzip compression is only useful for text based content.
	 * Ant-style patterns are supported, e.g. "text/*".
	 * 
	 * @see AntPathMatcher
	 */
	...
	/**
	 * Set the default path prefix to apply to resources being served from jar
	 * files. Default is "META-INF".
	 */
	...
	/**
	 * Set the number of seconds resources should be cached by the client. Zero
	 * disables caching. Default is one year.
	 */
