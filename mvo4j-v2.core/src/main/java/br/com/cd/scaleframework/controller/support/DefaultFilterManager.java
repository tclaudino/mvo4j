...
public class DefaultFilterManager<T> implements FilterManager {
	...
	public FilterManager addField(String fieldName) {
		this.addField(fieldName, LikeCritirion.NONE);
		return this;
	}

	public FilterManager addField(String fieldName, LikeCritirion likeType) {
	...
	public FilterManager setValue(String fieldName, Object value) {
		return this.setValue(fieldName, value, LikeCritirion.NONE);
	}
		...
		if (!map.isEmpty()) {
			...
			try {
				...
				if (controller.getInitialPageSize() > -1) {
					entityList = controller.getService().findList(map,
							controller.getOffset(), controller.getPageSize());
				} else {
					entityList = controller.getService().findList(map);
				}
