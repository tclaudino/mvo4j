...
public class DefaultController<T, ID extends Serializable> extends
		AbstractPageableController implements Controller<T, ID> {
	...
	protected ControllerBeanConfig<T, ID> config;
	...
	@Override
	public ControllerBeanConfig<T, ID> getControllerConfig() {
		return config;
	}

	@Override
	public final void resetPager() {
		this.setRecordsCount(getService().getListCount());
	}
	...
	public T newEntity() throws InstantiationException, IllegalAccessException {
		return config.targetEntity().newInstance();
	}
