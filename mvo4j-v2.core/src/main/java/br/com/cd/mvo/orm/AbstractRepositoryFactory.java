...
public abstract class AbstractRepositoryFactory<F, B, R extends Repository<?, ?>> implements RepositoryFactory<F, B, R> {
	...
	private static final String BEAN_NAME_PREFIX = AbstractRepositoryFactory.class.getName();
	...
	public AbstractRepositoryFactory(Container container, Class<? extends Annotation> persistenceTypeAnnotation,
	...
	protected abstract B createPersistenceManager(F factory) throws NoSuchBeanDefinitionException;

	private ReferenceMap cachedBean = new ReferenceMap();

	@Override
	@SuppressWarnings("unchecked")
	public final B getPersistenceManager(String persistenceManagerFactoryBeanName) throws NoSuchBeanDefinitionException {

		if (!cachedBean.containsKey(persistenceManagerFactoryBeanName)) {
			synchronized (this) {
				if (!cachedBean.containsKey(persistenceManagerFactoryBeanName)) {
					B persistenceManager = this.createPersistenceManager(container.getBean(persistenceManagerFactoryBeanName,
